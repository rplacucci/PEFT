...
from transformers import BertForSequenceClassification, BertTokenizer, DataCollatorWithPadding, logging
from .utils import tokenize_fn, postprocess_fn

# accelerate launch -m baseline.test
...
parser.add_argument("--task_name", type=str, default="qqp", help="Name of the GLUE task to train on", choices=["mnli-m", "mnli-mm", "qqp", "qnli", "sst2", "cola", "stsb", "mrpc", "rte", "wnli", "ax"])
...
# Config directories
out_dir = f"./outputs/baseline/submission-{checkpoint}"
...
# Load model
model_path = f"./models/baseline/{checkpoint}-glue-{"mnli" if task_name in ("mnli-m", "mnli-mm", "ax") else task_name}"
num_labels = 3 if task_name in ("mnli-m", "mnli-mm", "ax") else 1 if task_name == "stsb" else 2
