...
# accelerate launch -m baseline.experiment
...
# Config argparser
parser = argparse.ArgumentParser(description="Run experiments on BERT for a GLUE task (vanilla)")
...
parser.add_argument("--top_k", type=int, default=4, help="Top k layers to finetune on")
parser.add_argument("--lr", type=float, default=2.5e-5, help="Maximum learning rate for the optimizer")
...
# Config directories
run_id = f"{checkpoint}-glue-{task_name}-top_k-{top_k}-n_epochs-{n_epochs}-seed-{seed}"
...
for i in range(num_layers - top_k, num_layers):
    for param in model.bert.encoder.layer[i].parameters():
        param.requires_grad = True

# Unfreeze pooler and classification head
