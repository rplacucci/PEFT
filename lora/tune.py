...
# Config directories
log_dir = f"./logs/lora/{task_name}"
os.makedirs(log_dir, exist_ok=True)

out_dir = f"./models/lora/{task_name}"
os.makedirs(out_dir, exist_ok=True)
model_save_path = os.path.join(out_dir, f"{checkpoint}-glue-{task_name}-rank-{rank}-alpha-{alpha}-batch_size-{batch_size}-lr-{lr:.0e}-n_epochs-{n_epochs}")
...
accelerator.init_trackers(f"{checkpoint}-glue-{task_name}-rank-{rank}-alpha-{alpha}-batch_size-{batch_size}-lr-{lr:.0e}-n_epochs-{n_epochs}")
